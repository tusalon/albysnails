...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '68417e75-2b38-492b-a0a8-c6ccafe85ce7'; // ID de Albys Nails
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Albys Nails';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '55346987';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'amojenaf@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Albys Nails!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'albysnails';
};
...
console.log('✅ config-negocio.js listo para Albys Nails');
